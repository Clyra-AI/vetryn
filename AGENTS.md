...
- Passing review records require role-bound GitHub review evidence authenticated against the public GitHub
  API, the exact candidate executor, and a reviewer distinct from that executor. This fail-closed control-plane
  read uses no repository credential. Non-baseline evidence must match the reviewed plan and lockfile digests.
