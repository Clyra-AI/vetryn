...
6. When the packet sets `code_review_required`, require a candidate-bound Factory `review_report` that cites the
   exact validation report and has no unresolved blockers. Verify that neither product nor contract-bearing bytes
   changed afterward. Record pass or changes-requested evidence for maintainer review. Do not change product code
