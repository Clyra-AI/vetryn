...
2. Require one exact candidate commit, gate-bound passing command evidence for every active command criterion and
   ...
   historical provenance. A promotion commit may change only canonical state, that task's ledger status/evidence,
   compact new evidence, and generated progress. Stop rather than manufacturing or inferring evidence.
...
6. From the clean final pushed head, run `pnpm plan:check` and `pnpm check`, inspect the generated next-legal set and
   the full promotion tail, and prove that the tail changed only canonical state, that task's ledger entries,
   compact evidence, and generated progress. A product- or contract-bearing tail invalidates the review and must
   return to implementation. Then use Factory's `commit-push` in `land` mode to monitor the existing PR through CI,
   passive remote Codex review, merge, and post-merge validation. Acceptance does not imply merge, and merge does
   not retroactively supply missing evidence.
