...
6. Prepare candidate-bound, redacted evidence and hand off the exact commit to an independent verifier. Do not mark criteria accepted, approve a review role, edit generated `progress.json`, merge, or promote your own work.

Use Factory's `task-executor` when an agent worker is available. Use Factory's `commit-push` only after independent verification and maintainer authorization; this skill does not duplicate or weaken its GitHub lifecycle gates.
